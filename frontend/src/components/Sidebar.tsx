...
const Sidebar = () => {
  ...
  const fetchStaffAccounts = async () => {
    try {
      ...
      const roleOrder: Record<string, number> = { admin: 1, coordinator: 2, senior_leader: 3, leader: 4, student: 5 };
                  ...
                  <div className="bg-white border border-slate-100 rounded-xl overflow-x-auto shadow-sm">
                    <ul className="divide-y divide-slate-100 min-w-max w-full">
                      {staffAccounts.map(staff => (
                        <li key={staff.id} className="flex items-center gap-4 px-4 py-3 hover:bg-slate-50 transition-colors">
