...
async function main() {
  ...
  // Try to find admin table (case-insensitive)
  for (const row of tables.rows) {
    const name = row[0] || row.name;
    if (String(name).toLowerCase().includes('admin')) {
      console.log('\nFound admin table:', name);
      const rows = await client.execute(`SELECT id, username, role FROM "${name}"`);
      console.log('Accounts:', JSON.stringify(rows.rows, null, 2));
    }
  }
}
